...
/**
 * PAINT CALCULATOR
 * Based on 12m2 per litre (standard) and 2 coats.
 */
export const calculatePaintVolume = (totalArea) => {
  const coveragePerLitre = 12; 
  const coats = 2;
  
  const totalLiters = (totalArea / coveragePerLitre) * coats;
  
  return {
    liters: totalLiters.toFixed(2),
    drums15L: Math.ceil(totalLiters / 15), // 15L is the standard trade drum size
    tins4L: Math.ceil(totalLiters / 4)
  };
};
/**
 * STEP 4: QUOTE ENGINE
